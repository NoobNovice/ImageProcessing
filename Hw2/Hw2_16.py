...
if __name__ == '__main__':
    ...
    print("\nFourier Transform lenna image")
    cross_img = get_metrixImage(dir_path + "/Image/Lenna.pgm")
    N = len(cross_img)
    F = get_F(N)
    ...
    save2pgm(re_amp,dir_path + "/Image/restoreAmplitudeLenna.pgm")
    image = cv2.imread(dir_path + "/Image/restoreAmplitudeLenna.pgm",0)
    ...
    for i in range(len(re_phase)):
        for k in range(len(re_phase)):
            re_phase[i][k] *= 100
    save2pgm(re_phase,dir_path + "/Image/restorePhaseLenna.pgm")
    image = cv2.imread(dir_path + "/Image/restorePhaseLenna.pgm",0)
    cv2.imshow('image',image)
    cv2.waitKey(0)
    print("#######################################################")
